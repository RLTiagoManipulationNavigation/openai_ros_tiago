...
class TiagoEnv(robot_gazebo_env.RobotGazeboEnv):
    ...
    def _check_odom_ready(self):
        ...
        while self.odom is None and not rospy.is_shutdown():
            try:
                self.odom = rospy.wait_for_message("/mobile_base_controller/odom", Odometry, timeout=5.0)
    ...
    def _check_camera_depth_image_raw_ready(self):
        ...
        while self.camera_depth_image_raw is None and not rospy.is_shutdown():
            try:
                self.camera_depth_image_raw = rospy.wait_for_message("/xtion/depth_registered/image_raw", Image, timeout=5.0)
    ...
    def _check_camera_depth_points_ready(self):
        ...
        while self.camera_depth_points is None and not rospy.is_shutdown():
            try:
                self.camera_depth_points = rospy.wait_for_message("/xtion/depth_registered/points", PointCloud2, timeout=10.0)
    ...
    def _check_camera_rgb_image_raw_ready(self):
        ...
        while self.camera_rgb_image_raw is None and not rospy.is_shutdown():
            try:
                self.camera_rgb_image_raw = rospy.wait_for_message("/xtion/rgb/image_rect_color", Image, timeout=5.0)
